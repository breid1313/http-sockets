...
can simply install the Python modules ships with this repository using `pip`, and immediately being using the client within that environment.
...
locally-running server or externally to another server. We specify with AF_INET that we wish to communicate with IPv4 addresses and also
...
makes and ungraceful exit and the socket from the previous run remains open.
...
request protocol. For example, if the user is requesting index.hmtl from www.cnn.com over port 80:
...
After the basis of the request has been formed, we must add the file data to the body of the request (after the double carriage return / line feed).
We do this in a convenient way using Python's context manager (`with open("file.txt", "r"), as f:`) to easily open the file, steam the data to
...
yet design a way to only request data from just after the last received byte. This is ideal if there is large amounts of data being transferred from
...
first about the practical solution. If this were a real client going out to customers, they would want to be able to select any file from their system.
...
this assignment). However, this program is really about testing the HTTP client's ability to send data to another host and receive a response back.
...
it's not unreasonable to ask them to upload those files there.

Finally, you may have noticed the decision to allow the client to reuse the same address, effectively allowing to kill another socket that might be running
...
We now enter the infinite loop, listening on the socket. When a connection comes in, it is accepted and data begins streaming in to the server.
...
is interrupted, we raise this exception because we may or may not have received enough data to know what to do with the request.
...
file, we send a 201 Content created messgae, otherwise we send 200 OK, indicating that the resource was updated successfully. Again, these messages
sent UTF-8 encoded and the connection is closed.
...
keep the server environment more contained and aides in debugging and segmenting connections since the server is not configured to do any
...
nice improvements to make. First and foremost, caching would be a great improvement. A very simple idea would be to store the last 100 or
most commonly requested documents on the client with a time last modified for the document. With this information, we could send an if-modified-since
